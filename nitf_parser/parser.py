...
class NitfParser:
    ...
    def __parse_header(self, header_element):
        ...
        _object = self.__merge(_object, self.__parse_metadata(meta_data_list))

        # Docdata contains publishing date
        doc_data = header_element.getElementsByTagName('nitf:docdata')[0]
        _object = self.__merge(_object, self.__parse_docdata(doc_data))

        # Pub_data contains publisher and publication
        pub_data = header_element.getElementsByTagName('nitf:pubdata')[0]
        _object = self.__merge(_object, self.__parse_pubdata(pub_data))

        return _object

    @staticmethod
    def __parse_metadata(metadatas):
    ...
    @staticmethod
    def __parse_body_content(content):
    ...
    def __parse_body(self, body_element):
        ...
        _object = self.__merge(_object, self.__parse_body_head(body_element.getElementsByTagName("nitf:body.head")[0]))
        # body content contains the paragrapgs and subheaders
        _object = self.__merge(_object, self.__parse_body_content(body_element.getElementsByTagName("nitf:body.content")[0]))
    ...
    def parse(self, path):
        ...
        _object = self.__merge(_object, self.__parse_header(header_element))

        body_elements = item_list[0].getElementsByTagName('nitf:body')

        if len(body_elements) != 0:
            _object = self.__merge(_object, self.__parse_body(body_elements[0]))

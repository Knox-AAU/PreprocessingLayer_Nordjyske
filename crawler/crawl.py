...
from nitf_parser.__main__ import parse
...
def run_crawler(arg_object):
    ...
    folders = manage_folder_cache(arg_object)
    folders = check_and_filter_dates(arg_object, folders)
    ...
    for folder in folders:
        ...
        save_to_json(output_this_folder, f"{arg_object.output_folder}/{name}.json")


def manage_folder_cache(arg_object):
    ...
    if arg_object.clearcache or not path.exists(config['structure']['cache_file']):
        ...
        folders.sort(key=lambda folder: date_to_int(folder))
        save_to_json(folders, config['structure']['cache_file'])
    else:
        # load folders from folders.json
        folders = load_from_json(config['structure']['cache_file'])
    return folders


def check_and_filter_dates(arg_object, folders):
    ...
    int_from_date = date_to_int(arg_object.from_date)
    int_to_date = date_to_int(arg_object.to_date)
    if int_from_date > int_to_date:
        raise Exception("The to-date is before the from-date.")

    # filter folders by dates
    to_date = date_to_int(arg_object.to_date)
    from_date = date_to_int(arg_object.from_date)
    folders = [folder for folder in folders if from_date < date_to_int(folder) < to_date]
...
def find_folders_recursively(directory):
    ...
    for curr_dir in dirs:
        ...
        if matched_regex is not None:
            ...
            found_folders.extend(find_folders_recursively(directory+"/"+curr_dir))
    return found_folders


def save_to_json(folders, file_name):
...
def find_relevant_files_in_directory(directory):
    ...
    found_files = []
    for file in files:
        if is_string_in_list(file, blacklist) or not is_string_in_list(file, whitelist):
            continue
        if ".xml" in file:
